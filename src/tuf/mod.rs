...
impl SigstoreRepository {
    ...
    pub fn new(checkout_dir: Option<&Path>) -> Result<Self> {
        ...
        let metadata_base = url::Url::parse(constants::SIGSTORE_METADATA_BASE).unwrap();
        let target_base = url::Url::parse(constants::SIGSTORE_TARGET_BASE).unwrap();
    ...
    #[inline]
    fn tlog_keys<'a>(tlogs: &'a Vec<TransparencyLogInstance>) -> impl Iterator<Item = &'a [u8]> {
        tlogs
            .iter()
    ...
    #[inline]
    fn ca_keys<'a>(
        cas: &'a Vec<CertificateAuthority>,
        allow_expired: bool,
...
impl Repository for SigstoreRepository {
    ...
    fn fulcio_certs(&self) -> Result<Vec<CertificateDer>> {
        ...
        let certs: Vec<_> = certs.map(|v| CertificateDer::from(v)).collect();
